...
- **Duplicated transport plumbing across 4 `MatchTransport` implementations** (`socket_transport.dart` host+guest, `firestore_transport.dart`, `in_memory_transport.dart`): a `TransportChannels` mixin in `match_transport` carrying the three broadcast controllers + guards + `dispose()` — this is also where `_HostSocketTransport._setStatus`'s missing dedup (vs the other three) gets fixed for free.
  - **As implemented:** the dedup finding was backwards. `FirestoreTransport` was the only one that already skipped a no-op status write; the other **three** — the LAN host and guest halves and `InMemoryTransport` — re-announced the state they were already in. Reachable in two of them (the guest's `_onState`, and every `InMemoryTransport.simulate*` call); the **host's** was latent, its one `_setStatus` caller already sitting behind a `_connected` guard. All four now dedupe on `(status, reason)` through the mixin.
...
- **`native/engine_shim`**: `#[cfg(test)]`-gate or delete `wildbg_new` (only remaining caller is a vendored test — port it to `wildbg_new_with_path`); fix the one export not following the file's own `#[unsafe(no_mangle)]`/`# Safety`-doc convention.
  - **As implemented:** **gated**, not deleted-and-ported. The port was tried and abandoned: the vendored test names the EXACT move each match score picks, and the production nets play that position differently, so re-pointing it meant rewriting what it asserts. `wildbg_new` is now `#[cfg(test)]` — absent from the shipped cdylib, present for that test, which stays verbatim and green.
...
### Task 1.5 (added from live feedback): Firebase Analytics, Performance, Crashlytics + GitHub-issue feedback
Verbatim: "add app analytics using firebase so we can track usage, performance, and issues, add a feedback option that opens the github issues add form".

**Architecture reversal, deliberate and user-approved:** Plan 5 chose a pure-Dart REST transport specifically to avoid FlutterFire, because FlutterFire's Windows desktop support is partial/unofficial and this project does all its local dev/testing on Windows. Asked directly, the user chose real FlutterFire over a REST-only alternative (GA4 Measurement Protocol) because Performance Monitoring and Crashlytics have no clean REST ingestion path — SDK-only pipelines. This is now the standing decision; update `docs/superpowers/specs/2026-07-24-aigammon-architecture-design.md` and memory accordingly, do not re-litigate it in review.

- Add `firebase_core` + `firebase_analytics` + `firebase_performance` + `firebase_crashlytics` to `app/pubspec.yaml`.
- **Windows/desktop guard**: initialize Firebase only on Android/iOS (and web if ever targeted) — Windows/Linux/macOS builds must no-op cleanly, not crash, not even attempt `Firebase.initializeApp()`. Route through the existing desktop-gate pattern (`app/lib/main.dart` already distinguishes mobile vs desktop for the engine FFI loader — mirror that).
- **Config**: do NOT introduce `google-services.json`/`GoogleService-Info.plist` if avoidable — construct `FirebaseOptions` programmatically from repo Variables/dart-defines, following the exact pattern already established for `AIGAMMON_FIREBASE_PROJECT`/`AIGAMMON_FIREBASE_API_KEY`. New values needed (Android/iOS App IDs already exist as `FIREBASE_ANDROID_APP_ID`/`FIREBASE_IOS_APP_ID` secrets from App Distribution setup — reuse; a Measurement ID / sender ID will be new). Document every new repo Variable/secret needed in `firebase/DEPLOY.md`, mirroring its existing table format.
- **Analytics events**: meaningful usage signals — screen views (home, new-match, game, analysis, history, settings, lan, online), match started (mode: vs-AI/hot-seat/tabletop/LAN/online, difficulty, match length), match completed (winner, score, mode), tutor hint used, resign, cube actions. Keep the event/param naming consistent and documented in one place (a small `app/lib/analytics/` module) rather than scattered ad-hoc calls.
- **Performance**: `firebase_performance` custom traces around the operations that matter for this app specifically — engine move computation (AI think time), match transport connect/join latency, app cold-start to first-frame.
- **Crashlytics**: wire as an ADDITIONAL sink alongside the on-device crash log built in Task 1 (`app/lib/diagnostics/crash_log.dart`), not a replacement — `FlutterError.onError`/`PlatformDispatcher.instance.onError`/`EngineService`'s isolate error listener all forward to both. Non-fatal Dart errors and native crashes now get real dashboards; the on-device log remains the offline/no-network fallback.
- **GitHub issue feedback button**: independent of the Firebase question — add `url_launcher` (or confirm equivalent already available), a "Send feedback" affordance (Settings screen or the new Diagnostics screen from Task 1) that opens `https://github.com/tony-xmelon/aigammon/issues/new` with a pre-filled template (app version, platform, and a placeholder for the user's description) via query params.
- Tests: analytics/performance/crashlytics calls behind an injectable seam so they're testable without a real Firebase backend (an interface + a fake, same pattern as `MatchTransport`); the desktop no-op guard has a test asserting no Firebase call is attempted on a non-mobile platform; the feedback button's URL construction is a pure-Dart testable function.
- **Commit(s):** deps + desktop guard + config plumbing as one; analytics event module as one; performance traces as one; Crashlytics wiring as one; feedback button as one.
