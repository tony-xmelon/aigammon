...
- [x] Steps: failing widget tests → implement → green → commit `feat(buddy): the buddy game screen and home entry`.

**As implemented (Task 13): six slots, one roll said once, and a camera two screens were about to take from each other.**

*App suite 952+7skip → 970+7skip; `flutter analyze --fatal-infos` clean; `packages/` untouched.*

- **The screen is six bands, and four of them have a fixed height.** Top to bottom: the **readability light** (a colour, an icon and `Readability.message` verbatim), the **camera preview**, the **belief mirror** (the app's own `BoardView`, read-only except for tap-correct), the **prompt** (one sentence saying what Buddy is waiting for, plus whatever buttons answer it), the **transcript**, and the **control bar**. Only the two pictures flex; the light, the prompt, the transcript and the bar are `SizedBox`es, for the digital game screen's own reason — a match where the board resizes because a sentence got longer is a match played on a moving target. There is no game logic in the file: the one predicate that looks like some (`_canDouble`) is `game_hud.dart`'s gating, copied for the reason that file gives, with the controller still the authority behind it.

- **The prompt band is the answer to "where do the modal questions go".** One slot renders whichever of the session's questions is live — take/drop, which of two candidate plays, "type the roll instead", the objection Buddy just spoke — so the cube negotiation and the disambiguation picker are not dialogs that cover the board and are not extra height that moves it. The cube's buttons are **Take and DROP**, not the digital dialog's Take and Pass: Buddy has just said "I double — take or drop?" out loud, and the buttons under a spoken question have to be the words in it.

- **THE STUTTER, fixed at the policy level.** Buddy announced its roll twice — `onDiceRead` said "I rolled 6-6." and `onBuddyMoveChosen` followed a beat later with "I rolled 6-6 — play 1/7(2), 12/18*(2)". One throw, two lines, in the channel that IS the user's record of the match. `OpponentPolicy` now latches the roll it announced for Buddy and the dictation spends it: the transcript reads "I rolled 6-6." / "Play 1/7(2), 12/18*(2)". The dice-carrying form is **kept rather than deleted** — the dice are an argument of the method, and a caller that dictates a play with no throw in front of it deserves a line that says which throw it belongs to — and it is pinned by a test that calls the policy directly, along with the latch not being spent by the *user's* roll. Pinned from both ends: five cases in the new "one roll, one line" group and, through the screen, "says the roll ONCE a turn, not twice". **A second bug fell out of it:** the friendly phrasing was producing "I rolled 6-3 — play Move one checker from 13 to 8", because `describePlay` hands back a finished imperative sentence in that phrasing and the line put a verb in front of it. The dictation now takes the sentence as it stands and only terse gets the "Play " prefix.

- **DECISION, made out loud as Task 12 asked: the dice pad stays reachable while the light is red.** The pad answers the USER, not the camera — an outage suspends what perception may *claim*, never what a person at the board may *do* — and the worst case this mode is allowed to degrade to is a slightly assisted hot seat, which it only reaches if the manual route stays open when the camera is out. The banner stays up throughout, so the user knows why Buddy has gone quiet rather than thinking it has died, and the way back to a working camera is one tap away in the same band. Tested end to end through the screen: with a stale-calibration red on the very first frame, three taps on the pad start the match. The pad's *enabled-ness* is `BuddySession.awaitingRoll` (new, and exactly the condition `enterDiceManually` will accept) rather than a phase, because **no phase says both "a roll is open" and "the light is out"** — the session is parked in `calibrating` while the request is still pending.

- **The dance auto-pass HAS a test now, and it needed no seeding seam.** Task 11 recorded that it could not test the path because the session builds its controller from the standard starting position. It does not need to be handed a position: the dice are scripted and both sides play their first legal move, so a Buddy match is **deterministic given a dice script**, and a short search over scripts found that `5-1` then `6-6` puts the user on the bar against a home board they cannot enter — in five frames. The screen test drives those five frames and asserts the transcript says "No play — your turn passes." exactly once. (Determinism through the real `AiAgent` comes from `Difficulty.expert`, where `pickMove` takes `ranked.first`, plus a facade that ranks the real legal moves flat — the same shape `home_test`'s fake has.)

- **A camera bug the handoff would have shipped, found by writing the recalibration test.** `PhoneBuddyCamera` was opened and closed unconditionally by whichever screen had it, and the two screens' lifetimes overlap **in both directions**: the calibration route pops into the game screen (which opens before the popped route disposes), and the game screen pushes the calibration route back for a recalibration (which closes as it pops, under a screen still playing a match). Whichever disposed second took the camera away from the one still using it, and the failure is a preview that goes black with nothing logged anywhere. `open`/`close` are balanced calls now and only the last `close` tears anything down; `shutDown` (the provider's teardown, not a screen's) ignores the count. `FakeBuddyCamera` counts the same way — a fake that shut its frame stream on the first close would have made the resume untestable, which is how the real one would have shipped.

- **`BuddySession` grew three small things a screen needs and nothing else.** `awaitingRoll` (above); `recalibrate()`, the user-requested outage — identical in every respect to a stale calibration, which is the point, and the caller `Readability.stated`'s doc anticipated; and `useCalibration(vision, {seat})`, because **the seat is confirmed again by every recalibration** and `seat` was final. A user who moves round the table has a different near half, and that is the whole of what tells the next game's two opening dice apart. The field is now mutable with the same kind of doc `BuddySpeaker.phrasing` carries, and `useCalibration` is what moves it: a re-confirmed seat and a fresh calibration are one answer, not two.

- **The opening throw is the one roll whose ORDER matters, and the pad now says so.** `OpeningRollEvent(whiteDie: dice.die1, blackDie: dice.die2)` decides who starts, so on that throw the pad's two rows are two different people's dice and are labelled **"White's die" / "Black's die"**; on every other roll both faces belong to whoever is on turn, and the rows say "One die" / "The other". No new session API — the labels are the whole fix. The pad is a bottom sheet with two rows of six and a deliberate **Enter roll**: three taps rather than two, because what it returns goes straight into the authoritative game state and a roll is not a selection.

- **Tap-correct is the mirror in `BoardView`'s interactive mode**, live exactly while the session is holding the user's turn open (`awaitingPlay`, `objecting`, `disambiguating`), committing through `BuddySession.enterPlayManually` and never onto a board. While it is live the control bar shows **Undo / Confirm** in place of Dice / Double, which costs nothing: during the user's own move no roll is open and doubling is over for the turn, so the two buttons it replaces are two disabled buttons.

- **The home entry is hidden on desktop through `isBuddyModeSupportedPlatform`**, a `defaultTargetPlatform` getter of the same shape as `isFirebaseSupportedPlatform` and `isBuddySpeechSupportedPlatform`. Deliberately a second getter rather than a use of the speech one: they are different questions ("is there a voice", "is there a camera to watch a board with") and one of them would move if `flutter_tts` grew a desktop implementation. The button sits with the two other LOCAL modes (the opponent is the engine and the board is the one on the table) rather than with the two remote ones.

- **`buddyTtsProvider`, and why a provider was needed at all.** `flutter_test` runs with `defaultTargetPlatform == TargetPlatform.android`, so `BuddySpeaker.forPlatform` builds a real `FlutterTts` in every widget test that mounts this screen. The platform choice itself is unchanged and still lives in `speaker.dart`; the provider is the seam that lets a test hand over `SilentBuddyTts`.

- **A Buddy match lands in History like every other**, through `RepositoryPersistence` and a `mode: 'buddy'` match row created in `initState` — the same three lines `NewMatchScreen._persistenceFor` runs. Without it the one mode played on a real board would be the one mode that left no record.

- **The tidy Task 12 flagged is done, and it stayed a pure move.** `ObservedFrame` lives in `app/lib/buddy/observed_frame.dart` — three fields and a `Frame`, importing nothing but `board_vision` — so `buddy_session.dart` no longer pulls `package:camera` and `package:sensors_plus` in behind it. `camera_frame_source.dart` re-exports it, so no existing import changed. **It is not plugin-free, though, and the fix round corrected this note for saying so by implication:** the session imports `buddy_policy.dart`, which imports `speaker.dart`, which imports `flutter_tts`. That chain is the *policy* seam rather than the perception one and it is guarded differently — `BuddyTts` is an interface, `SilentBuddyTts` is what a desktop and every test get, and the whole session suite runs on a machine with no TTS engine — but the import graph does still reach a plugin, and "no plugin behind the session" would be the wrong sentence to carry into Task 15.

- **Recorded, not fixed: backing out of the calibration flow leaves the session without a calibration.** Deliberate — the old vision is dropped the moment a recalibration is asked for, because a camera answering questions about a board somebody is re-aiming is worse than a camera saying nothing. A user who changes their mind lands back on the game screen with the light red, the way in one tap away, and the corners still seeded. **"And the dice pad still live" was too broad, and the fix round both narrowed it and earned most of it back:** the pad is live exactly while a throw is open, which is the pre-roll gate and nothing else. What the fix round added is that every OTHER user verb now survives the same outage too — take/drop, the candidate picker, tap-correct on the mirror and Double. So the honest version is: whatever the user was already being asked, they can still answer with the calibration gone; the one state that leaves the corner flow as the only way forward is a dictated move waiting to be placed, where the outstanding question is the camera's.

- **Not done here, deliberately.** No analytics and no `AnalyticsScreenView` wrapper (Task 14 owns Buddy telemetry, and `AnalyticsScreens` has no Buddy entry), no mic (Task 14), no version bump (Task 15). The camera preview carries no overlay, so the preview↔frame mapping Task 15 verifies first costs this screen nothing: a stretched or mirrored preview is still a picture of the board, and no coordinate is read off it.

**Task 13 review round: a readability outage suppresses PERCEPTION's answers, never the USER's.**

*App suite 970+7skip → 996+7skip; `flutter analyze --fatal-infos` clean; `packages/` untouched.*

- **The principle the round is named for was already in the file, applied once.** Task 13 argued it for the dice pad — the pad answers the USER, not the camera — and then gated every other affordance on `BuddySession.phase`, which a non-green frame overrides. So ONE amber frame took the whole match away: Take and Drop vanished with Buddy's cube still on the table, the candidate picker vanished with two plays still tied, Double went dead behind "You can only double before rolling, on your turn." while the user was standing at exactly that gate on exactly their turn, and a half-tapped correction was destroyed outright — `BoardView` rebuilds its move builder when `interactive` changes — under a prompt that says "Nothing is lost."

- **The mechanism: open QUESTIONS, not phases.** `BuddySession` gained `awaitingCubeAnswer` and `awaitingPlay` beside `awaitingRoll`, each one exactly the condition its own verb accepts (`_humanAgent.pendingCube`, `_humanAgent.pendingMove`), each read from the pending question rather than from the phase, so none of them can be overridden by a light. The screen's prompt, its answer buttons, its mirror's `interactive` and its Double all read those. The outage sentence became the FALLBACK: what is left to say when the only thing outstanding is something the camera has to answer — pinned from both ends, since a `_userQuestionLine()` that swallowed the pause line entirely would be its own bug.

- **DECISION on the F2 fork, recorded as asked: Double stays ENABLED through an outage** rather than staying disabled with a truthful sentence. Doubling is the user speaking; the cube in the middle of a real table is not a claim about a picture. Its gate is now `awaitingRoll` — the open throw — in place of the phase, which also makes every clause of the blocked-reason sentence a clause the button actually refuses on.

- **`_canDouble` became the top-level `buddyDoubleAvailable`**, which is the sixth copy of `GameController`'s private `_doublingLegal` and the first that a test can put a real controller in front of. Pinned the way the session's copy is, in the same group and against the same authority — centred, Crawford, user owns, buddy owns, cubeless, plus the rewritten clause read away from the gate. Verified by mutation: dropping `|| s.cube.owner == s.turn` — the drift that fails silent, since a button that has merely stopped lighting up throws nothing — turns the owner test red.

- **A decided match closes the throw it was waiting for.** The cube is the one verb that ends a game FROM the pre-roll gate, which is exactly where a physical throw has already been asked for, so `awaitingRoll` outlived the match: the game-over screen kept a live Dice button whose answer would resume `_requestRoll`, announcing a roll after the last point and calling `rollDice()` on a finished game inside an unawaited future. Closed at the question (`awaitingRoll` reads `matchOver`; `enterDiceManually` refuses on the same getter) rather than by cancelling the roller — which is the Task-11 deadlock, since `cancel` never completes the future `_requestRoll` is suspended on and the in-flight flag its `finally` clears would stay set for good. Reaching the state needs a two-point match, a cube Buddy turns, a take that puts it in the user's hands at 2 and a redouble Buddy drops; a correct engine never drops there (it loses the match outright), so the harness scripts the answer and says so at the flag.

- **`useCalibration` drops the outgoing epoch's frames by itself.** `recalibrate()` then `useCalibration()` was a two-call protocol with nothing enforcing the first call, and a caller installing a fresh calibration on its own left the old "before" frame in place — after which the next play query differences two pictures of two different boards, quietly, since `board_vision` says it cannot check that itself. The two verbs are idempotent with each other now rather than ordered.

- **`CameraHold`, and an interface doc that had gone false.** `BuddyCamera.close()` still promised it was "safe to call more than once", true of the unconditional teardown the count replaced and false since: an extra close releases somebody else's hold. The rule now lives on the interface. And the count itself was untested, sitting below the plugin-edge line where the policy exempts plugin CALLS — which counting is not — so it is `CameraHold` above the line, pure arithmetic with `PhoneBuddyCamera` reduced to three one-line calls. Verified both ways it can be got wrong: dropping the `> 0` guard drives the count negative and swallows the next real close; returning true unconditionally is the original bug.

- **Both flexible slots hold at any text size.** `_Placeholder`'s paragraphs are the longest sentences on the screen and they live in the two bands that flex; the camera-refusal one overflowed a 420x900 phone at TextScaler 1.3 by 36 pixels. It wraps at the width its slot has and a `FittedBox` shrinks the wrapped block — the width constraint being the load-bearing half, since an unconstrained `Text` inside a `FittedBox` lays out on one endless line and is then scaled to nothing. Shrunk rather than ellipsized: both sentences tell the user what to do next.

- **Four claims nothing was reading back** now have tests: the screen's own `mode: 'buddy'` History row (through `runAsync`, because the write is real sqlite I/O), the non-opening pad's "One die" / "The other", Drop's presence beside Take, and the camera handover in the direction the recalibration test does not cover — the corner flow popping INTO a game screen that opened before it disposed, driven through the real `openBuddyGame`.
...
- [x] Steps: failing tests → implement → green → commit `feat(buddy): mic attention trigger, telemetry, phrasing setting`.

**As implemented (Task 14): a hint that can only shorten a wait, one flag that is both a preference and a refusal, and five events that never fire per frame.**

*App suite 996+7skip → 1055+7skip; `flutter analyze` clean; `packages/` untouched; no version bump (Task 15 owns it).*

- **The seam is one method, and it touches exactly one of the gate's three rules.** `FrameGate.attend()` sets a flag; the next `offer` converts whatever the throttle would have said, and then runs at `kAttentionInterval` (80ms) instead of `kObservationInterval` (250ms) for `kAttentionWindow` (1.5s). The signature, the quiet run and the gyro are untouched — they already run on **every** offered frame, before the thinning — so `isStable` is computed exactly as it would have been and a nudge can only PUBLISH a verdict the gate had already reached, never manufacture one. That is the whole safety argument for putting a microphone anywhere near perception, and it is a test rather than a sentence: *"a nudge never makes an unstable frame stable"* feeds a moving hand through a nudged gate and asserts every published frame comes out unstable. The gate stays clock-free — a nudge is a flag the next `offer` picks up rather than a timestamp a caller has to be able to produce — because every time the gate knows already arrives on a frame.
- **The inertness proof is a LIVE microphone, not an absent one.** *"a gate nobody nudges is the gate that existed before the microphone"* runs one 40-frame script through two gates — one bare, one wired to a real `DiceSoundListener` over a source delivering six seconds of room tone — and asserts the two published sequences are equal on `(at, isStable, sceneChange)`. A test with the microphone merely unplugged would have proved nothing; this one proves the detector can be subscribed, running and consuming audio and still be indistinguishable from its own absence. At the screen level the same claim is made where a user would feel it: the same scripted match plays to the same transcript and the same prompt with a refused microphone and with one that hears a throw.
- **The signature is IMPULSE versus SUSTAINED, and nothing cleverer.** A candidate transient starts when a hop jumps `kDiceAttackDecibels` (12dB) over a running room floor *from a hop that was still in it*; it FIRES if it falls back within `kDiceQuietDecibels` (6dB) of that floor inside `kDiceDecayWindow` (250ms), and is abandoned if it does not. Speech, a television and a passing car all fail the third test, because all three hold their energy for longer than a quarter of a second; the running floor (`kRoomFloorTimeConstant`, 1.5s) then climbs to meet a room that stays loud, so a noisy room stops being twelve decibels of news within a few seconds. `kDiceFloorDecibels` (-45dBFS) rejects a rustle that clears +12dB against a very quiet floor, and `kDiceRefractory` (700ms) bounds how often even a pathological room can spend a hint. **The six are Provisional in the frame gate's sense** — there is no board, no phone and no microphone here, so each ships with the arithmetic that produced it rather than a measurement, and Task 15's on-device protocol is where they get something to disagree with.
- **Two tests exist only to prove two of those constants are load-bearing.** The house style is easy to fake here: a detector that never fires passes every negative test in the suite. So *"the decay window is what rejects speech, and nothing else is"* runs the SAME sustained script through a trigger with a five-second window and asserts it fires, and *"the absolute floor is what rejects a rustle"* does the same with the floor removed. Without those two, "speech never fires" is compatible with "nothing ever fires".
- **The hint fires when the noise STOPS, not when it starts**, and that is the design rather than an accident. The moment worth looking at the board is the moment the dice have come to rest, which is also the moment the gate's own scene-quiet run can begin. Pinned by a test that asserts the fire lands after the attack and within one decay window of it.
- **Recorded, not fixed — `kQuietFramesRequired`'s doc arithmetic is wrong about the cadence, and it makes the nudge MORE useful rather than less.** That constant's doc reasons "three at `kObservationInterval` apart is ~0.75s of nothing happening", but `FrameGate.offer` updates the quiet run on every offered frame *before* the throttle, and a device offers at ~30fps. So three quiet frames is about 100ms on a phone, not 750ms, and the existing tests never caught it because they offer frames at exactly `kObservationInterval`. The consequence for this task: after the dice land, stability is reachable in about a tenth of a second and **the 250ms throttle is the dominant remaining wait** — which is precisely the wait `attend()` removes. Left alone because it is a documented Provisional constant whose real value is a Task 15 measurement, and because "correcting" it would mean changing gate behaviour on a guess. *(Fix round: the correction now lives AT the constant, where a reader of the code meets it, rather than only in this plan.)*
- **`record` 7.1.1, chosen for what it does NOT do.** `startStream` hands over raw PCM16 in memory; each chunk becomes a handful of RMS numbers through `PcmAmplitudeMeter` and is dropped, so **nothing is ever written to disk** — the alternatives that record to a file would have Buddy keeping audio of the room for the length of a match, which is not a trade an attention hint is worth. The three audio-processing options (`autoGain`, `echoCancel`, `noiseSuppress`) are switched OFF explicitly rather than left at their defaults: all three exist to flatten sudden changes in level, and a sudden change in level is the entire signal. Amplitude is computed here rather than taken from the plugin's own reading, which is documented in neither units nor cadence and differs between the Android and iOS implementations; the meter's clock is the **sample count**, so a late chunk and two chunks arriving together describe the same 16ms hops either way. **No Windows prerequisite**, unlike `flutter_tts`: `record_windows` builds against the Windows SDK's own Media Foundation headers and calls no `find_program` — verified by reading its `CMakeLists` rather than assumed — though it does want CMake 3.23, which the CMake bundled with VS 2022 satisfies. README's toolchain section now says both things beside the nuget note. **Its ANDROID build is a CI watch item, though** (fix round): `record_android` 2.1.2 builds itself with AGP 9.2.1 where `android/settings.gradle.kts` pins 9.0.1, and requires `compileSdk` 36 — exactly Flutter 3.44.8's `flutter.compileSdkVersion`, so zero headroom. Nothing local sees it (no Android toolchain on this machine, and a desktop build reads no Gradle file); `android.yml` assembles a real APK, so that workflow is where a plugin raising its own minimum surfaces, as a Gradle failure naming the version it wanted. Recorded in README beside the nuget note, with the fix if it fires: raise the pin or the channel, not the plugin back.
- **The permission is asked where the screen is already asking for a throw.** The opening throw is outstanding from the moment `BuddyGameScreen` mounts, so `_syncMic()` runs at the end of `initState` and the operating system's dialog arrives over a prompt that says "Throw the opening dice". It opens ONCE and stays open for the session rather than following each turn, for a reason that is not laziness: starting a PCM stream is an asynchronous platform call, so a microphone that opened at the start of each throw would routinely still be opening while the dice were landing — it would miss exactly the sound it exists for. It stops the moment the match is decided, and on dispose.
- **One flag, not two, and that was the design decision worth arguing.** `buddyMicHint` (schema v9, ON by default) is the user's preference AND the remembered refusal. Two flags — a preference plus a hidden "already refused" bit — would leave a user who refused once and later granted the permission in system settings with **no way back**: nothing on screen to turn on, and a mode that has promised never to ask again. One flag means the Settings switch a refusal turns off is the same switch that turns it back on. A refusal latches it through `SettingsRepository.markBuddyMicRefused()`, a single-column write for `markDragHintShown`'s reason and across a wider gap than that one: the session's settings snapshot was read when the match started. **Only a refusal latches** — `unavailable` (no microphone, no plugin) is a fact about the device that will be just as true next time without anybody being asked twice, and there is a test for each.
- **Schema v9 adds `buddy_phrasing` and `buddy_mic_hint`, gated `from >= 2 && from <= 8`.** Not `from < 9`, for the reason the v2 branch has been stating since v3: a v0/v1 database has its settings table created whole from the CURRENT definition and therefore already has both columns, so a `from < N` gate would `addColumn` over one that is there. The "every upgrade path" test is what catches that, and it gained a v8 rung (`_v8OnlineSessionDdl`; there is deliberately no `_v8SettingsDdl`, because v8 added no settings column and a second copy of the v7 shape could drift from its twin). The 8 → 9 test is the path every currently-installed build takes: every pre-existing value survives bit for bit, the online-session identity is not re-created over a signed-in user, and both new columns are written to afterwards — the failure a missing branch would produce at the first settings save.
- **`BuddyPhrasing` moved to `app/lib/buddy/phrasing.dart`, and `speaker.dart` re-exports it.** A pure move of `BuddyLine`, `BuddyPhrasing` and their private grouping helper, taking the exact precedent Task 13 set with `observed_frame.dart`: a value type everybody needs should not drag a plugin in behind it. Here the everybody is `lib/data/app_settings.dart` — the phrasing is a persisted preference now — and the plugin is `flutter_tts`, which the settings layer has no business importing to hold an enum with two values in it. No import anywhere else had to change.
- **The setup screen now READS the setting it was ignoring.** Task 12 left `_phrasing = BuddyPhrasing.terse` as a literal, which would have made the new preference a preference the screen it applies to ignores. It is seeded from `settings.buddyPhrasing` and **not written back**, per-match exactly like the match length and the difficulty beside it — so a user can change their mind for one match without changing their mind for good, and the test asserts both halves.
- **Five events, and the split between them is "what can afford to be per-occurrence".** `buddy_session_started` (mode, length, difficulty, cubeless, seat, phrasing, whether the hint is enabled) and `buddy_session_ended` (whether the match was decided, the readability-red RATE, what became of the microphone, how many hints it raised) fire once a session; `buddy_calibration_attempted` (ok, recalibration), `buddy_recalibration_entered` (whether the board was already lost) and `buddy_fallback_used` (`dice_pad` | `candidate_picker` | `tap_correct`) fire per occurrence, because a handful a match is not spam. **Nothing fires per frame**: readability is assessed four times a second, so `BuddySession` counts it and reports one ratio — over frames it actually ASSESSED, which excludes frames that arrived with no calibration, because a dead calibration cannot judge its own readability. *(Fix round: and thinned to one assessment per observation interval, so the nudge cannot move the number — see the review round below.)* Calibration is one event with an outcome parameter rather than two events, so the success rate is a ratio the console can compute rather than two counters that could drift apart.
- **The telemetry lives in the SCREENS, so `app/lib/buddy/` stays analytics-free.** All three fallbacks, the recalibration entry and both session events are called from `buddy_game_screen.dart`; the calibration attempt from `calibration_screen.dart`. The only thing the session gained is `readabilityRedRate`, which is arithmetic rather than a sink. The screen reads `appAnalyticsProvider` and `settingsRepositoryProvider` once in `initState` rather than at each call site, because two of the three uses are in `dispose`, where reading a provider is reaching into a scope that may already be tearing down. `AnalyticsScreens` gained the three Buddy screens — the funnel between them is the question worth asking — and `AnalyticsModes` gained `buddy`, mirroring what `MatchRepository` already files these matches under.
- **The widget-test guard is an override, not a platform check.** `flutter_test` reports android, so `isBuddyMicSupportedPlatform` answers yes and an un-overridden screen would build a real `AudioRecorder`. `buddyMicProvider` is a **factory** rather than an instance (a source is closed by the session that opened it, so a shared one would hand the next match a shut microphone) and both buddy screen harnesses override it along with `appAnalyticsProvider` — the same discipline `buddyTtsProvider` shipped with in Task 13. `DiceSoundListener.start` also catches everything and answers `unavailable`, so a suite that forgets is degraded rather than broken. The game-screen harness also moved to a `ProviderContainer` awaited before the pump, because the screen reads the settings synchronously in `initState` and a stream provider that has not emitted yet answers null.
- **One fake, in one file.** `test/buddy/fake_mic.dart` holds the microphone double and the two scripts (`quietRoom`, `diceClatter`) that three suites need — the detector's own tests, the gate's inertness proof and the screen's wiring. The reasoning is `fake_calibration_seams.dart`'s, applied one file further: a double that exists three times drifts in whichever two copies a fix was not applied to.
- **Not done here, deliberately.** No version bump and no CHANGELOG entry (Task 15). No performance trace for the detector — it is a few dozen floating-point operations per 16ms, and a trace would cost more than the thing it measured. No diagnostics surface for the room floor, though `DiceSoundTrigger.roomFloorDecibels` is public so Task 15 can put one behind a debug build if the on-device protocol wants to watch it.
- **Recorded, not fixed — what the detector cannot do.** It separates an impulse from a sustained sound, which is what separates dice from speech and from a television. (Kept as written: the fix round below touched the metric, not this trade.) It does **not** separate dice from other impulses: a knock at the door, a clap, a snare drum on the radio all fire it. That is the accepted half of the trade and it is stated at `kDiceDecayWindow`, because the cost is bounded and small — one early look at a board that has not changed, at most once per refractory — while the cost of the other error, a detector that fires through continuous speech, would be a hint that means nothing at all. What would change the verdict is the on-device measurement finding a room where impulses are common enough that the gate never leaves its fast cadence; the window and the refractory are the two knobs for that, and both are constructor parameters.

**Task 14 review round: a metric that was measuring the throttle, and four docs describing something other than what they sat on.**

*App suite 1055+7skip → 1057+7skip; `flutter analyze` clean; `packages/` untouched.*

- **The one behaviour change: `readability_red_rate` was confounded by the feature it shipped beside.** `BuddySession` counted every published frame, and `attend()` raises publication from 4/s to 12/s for 1.5s after each throw — a window that lands squarely on the hand withdrawing over the board, which is the reddest moment of a turn. So `mic_state: listening` sessions reported a systematically higher red rate than identical `off` ones, and the difference was the throttle rather than the room: the first Task 15 reading of the number would have looked like the microphone making rooms harder to see in. **The nudge is right and was not touched.** The metric now samples at most one assessment per `kReadabilitySampleInterval` — `kObservationInterval` restated where `buddy_session.dart` can see it (it may not import `camera_frame_source.dart`, which is what `observed_frame.dart` exists to prevent) and pinned equal by a test, because a drift would fail nothing and quietly re-confound the number. `framesAssessed` still counts every question put; only the rate's denominator is thinned. Red-first: a burst of red attention frames scripted into an otherwise identical session reported 0.5 against the silent session's 0.25, and both now report 0.25. The event still carries `mic_state`, which costs nothing and keeps the split available for a question nobody has asked yet.
- **Both frame fakes now advance a frame clock.** `Harness.stableFrame` and `FakeBuddyCamera.push` stamped every frame `Duration.zero` — a clock that never moved, which nothing read until the metric started sampling on one. They step an observation interval per frame, and `Harness.attentionFrame` is the other cadence: one extra frame an attention interval after the last, which is a frame the session sees only because something nudged the gate.
- **`kQuietFramesRequired`'s correction moved from this plan into the code.** The bullet above has said since Task 14 that its "~0.75s" is really ~100ms; a reader of the constant met the wrong arithmetic anyway. The doc now carries what it actually gates, why it is left at 3 (the correction cuts in the nudge's favour — the 250ms throttle dominates), and that Task 15 measures it with the sampling rate written down beside it, because a count of frames means nothing without one.
- **`markBuddyMicRefused` had been inserted INSIDE `markDragHintShown`'s doc block.** The new method carried a 21-line comment whose first two paragraphs describe the drag hint and refer to `[markDragHintShown]` in the third person, and `markDragHintShown` itself had no doc at all. Untangled; each method keeps its own.
- **The inertness proof's comment claimed more than the test did, so the test grew to match.** "Delivered before and during" was false — the audio drained entirely before `publishedFor` ran — so a third gate now takes the same six seconds ten hops at a time BETWEEN the frames, and the interleaved run is asserted equal to the bare gate's too. The comment also says what the equalities are worth: they are claims about an un-nudged gate, since the zero-hints expectation precedes them and a nudged gate publishes differently by design. A non-empty guard, so three empty lists cannot agree their way to a pass.
- **`buddy_session_started` does not mean "the camera is open".** It fires from `initState`, before the `unawaited(_open())` beside it comes back, so a session whose camera never opened is in the count. Left as it is and said out loud instead: it is the number of times a user got as far as starting a match, and a failed camera shows up in `buddy_session_ended` as a session that did not complete.
- **The `record` plugin's ANDROID build is a CI watch item** — AGP 9.2.1 against a 9.0.1 pin, `compileSdk` 36 against a Flutter that is exactly 36. Recorded in the `record 7.1.1` bullet above and in README's toolchain section, beside the nuget note that gets the same treatment. Nothing local can see it; `android.yml` builds a real APK, so CI is where it would land.
...
- [x] Step 1: Full matrix: `flutter test` (goldens included), `dart test` × all packages (now incl. `board_vision` + its corpus harness), `dart analyze --fatal-infos` × all, `cargo` suite, Firebase emulator pipeline (unchanged code but the ship-gate runs it per house rule).
...
**As implemented (Task 15, Step 1 + docs): the whole matrix green including the emulator pipeline, one shell trap that looked like a red leg and was not, and a protocol whose first item can invalidate all the others.**

*Version bumped to **0.14.0+19** in BOTH `app/pubspec.yaml` and `app/lib/branding/app_version.dart`; `app_version_test.dart` (the drift guard the plan warns about) run on its own and green. The `+19` follows the tracked sequence — `android.yml`/`ios.yml` override it with `--build-number=${{ github.run_number }}` at build time, so the pubspec number is a counter, not a promise.*

**The matrix, exactly as run.**

| Suite | Result |
|---|---|
| `flutter test` (app, goldens included — they are default-included per `app/dart_test.yaml`) | **1057 pass, 7 skip** |
| `dart test` `backgammon_core` | **140** |
| `dart test` `board_vision` (corpus harness included) | **521** |
| `dart test` `engine_bindings` (unit) | **59** |
| `dart test -P engine` `engine_bindings` (real DLL + production nets) | **17** |
| `dart test` `lan_play` | **145** |
| `dart test` `match_transport` | **104** |
| `dart test` `online_client` | **143** |
| `cargo test --release` (`native/engine_shim`) | **6 unit + 1 smoke** |
| `dart analyze --fatal-infos` × 6 packages, `flutter analyze` × app | **clean, no issues** |
| Emulator leg 1 — `firestore.rules` (mocha) | **110 passing** |
| Emulator leg 2 — `online_client -P emulator` | **52** |
| Emulator leg 3 — app two-client E2E (listener path + adversarial legs) | **7** |
| Emulator leg 4 — the same E2E on the poll fallback (`AIGAMMON_E2E_LISTEN=0`) | **1** |

Nothing regressed and nothing was skipped. The engine DLL was present, so the `engine` preset ran for real rather than being reported as unavailable.

- **The one red leg was the shell, and it is now written down where the next person will hit it.** `pwsh firebase/run-emulator-tests.ps1` — the invocation README gives — failed leg 2 REPRODUCIBLY on ``Could not find `bin\test.dart` in package `test` ``, before a test loaded. It is not the code and not `emulators:exec`: this machine's `pwsh` is the **Microsoft Store (MSIX) build**, which hands child processes a virtualized `%LOCALAPPDATA%`, so `dart` cannot see the pub cache that `package_config.json` points at (`dir "%LOCALAPPDATA%\Pub\Cache\hosted\pub.dev"` returns "cannot find the path" from inside a pwsh child, while the directory demonstrably exists). The identical command string from `cmd`, Git Bash or Windows PowerShell passes. `powershell.exe -File firebase/run-emulator-tests.ps1` ran all four legs green, and README's "Deploying online play" section now carries the trap and the workaround.
- **What this environment could NOT run, and who covers it.** No Android or iOS toolchain and no device here, so the **APK/IPA builds never happened locally** — `android.yml` and `ios.yml` assemble real artifacts on push and are where the `record_android` watch item (AGP 9.2.1 against a 9.0.1 pin, `compileSdk` 36 against a Flutter that is exactly 36) would surface, as a Gradle failure naming the version it wanted. Everything else in the matrix ran to completion. **CI covers: the Android APK, the iOS `.app`/IPA, and the Linux legs of everything above. The device protocol covers: everything with a lens, a microphone or a gyroscope in it.**
- **`docs/buddy-mode-test-protocol.md` opens with the seven unverifiable-here items, in priority order, and says so.** They are collected from Tasks 9–14's own As-implemented notes rather than invented: (1) the **preview↔frame mapping**, marked BLOCKING with a "if landscape fails, stop" instruction, because every number measured after it would be a number about a broken coordinate frame; (2) **`kOpeningSeatMargin`** (0.02) against real throws, scored as *correct / fallback / **wrong***, since a fallback is a nuisance and a wrong attribution is a corrupted game record; (3) the **six mic constants under a real AGC**, with `kDiceFloorDecibels` (−45 dBFS) called out as the device-sensitive one and an explicit false-positive budget (five noisy minutes, two silent ones — a silent room must fire zero); (4) the **Android APK build**, which is CI's to fail and the tester's to install; (5) the **permission dialogs landing over the right prompt** (camera at calibration, microphone over "Throw the opening dice"), including the refusal paths; (6) the **five frame-gate constants**, with the instruction to write the device's **frame rate** down FIRST because `kQuietFramesRequired` is a count of frames and means nothing without one; (7) the **VideoRange decode on a real iPhone**, tested as a same-board/same-light A/B against Android because the failure is a washed-out calibration rather than a crash.
- **The scripted match is twelve checkpoints plus one, and each says what failure looks like.** C1 the three preconditions (dice off the board) · C2 eight handles on a folding case and the twelve derived columns · C3 seat, capture, the belief drawn back · C4 the opening throw with the mic dialog behind it · C5 dice read (with tap-to-enter counted as a *documented fallback*, not a failure — the finding is the rate) · C6 the play observed and acknowledged · C7 dictation, then a deliberate misplacement to hear "That isn't quite it.", then three in a row to force the mirror escalation · C8 an illegal play and the objection · C9 a readability outage (a book, then the lamp) for the spoken-once red and "I can see the board again." · C10 a nudged board and the corner-nudge recalibration, with the score surviving · C11 the cube by voice and the Double button's gating · C12 a dance, the match end, the History row and analysis · C12b the terse/friendly switch. Every quoted line in the doc was taken from `buddy_policy.dart`, `readability.dart` and the two screens rather than paraphrased, so a tester comparing what they hear against the page is comparing against the code.
