...
String eventRows(List<(int seq, GameEvent event)> rows, {int gameNo = 1}) =>
    jsonEncode([
...
void main() {
  ...
  group('pollMatch', () {
    test('emits events once in seq order and stops on cancel', () async {
      final scripted = <String>[
        eventRows([(0, DoubleEvent(Player.white))]),
        eventRows([]), // nothing new
        eventRows([(1, TakeEvent(Player.black))]),
      ];
      var eventCalls = 0;
      final api = await apiFor((req) async {
        if (req.url.path.endsWith(':runQuery')) {
          final sq = (jsonDecode(req.body) as Map)['structuredQuery'] as Map;
          final collection = (sq['from'] as List)[0]['collectionId'] as String;
          if (collection == 'rolls') return http.Response('[]', 200);
          final body = eventCalls < scripted.length
              ? scripted[eventCalls]
              : eventRows([]);
          eventCalls++;
      ...
      await sub.cancel();
      final callsAtCancel = eventCalls;
      await Future<void>.delayed(const Duration(milliseconds: 20));

      expect(seen, [0, 1]);
      expect(eventCalls, lessThanOrEqualTo(callsAtCancel + 1),
          reason: 'polling must stop after cancel');
    });

    test('re-emits a roll only when its phase advances, then retires it',
        () async {
      // The same roll doc walks commit -> entropy -> reveal; afterwards the
      // watermark must move past it so it is never re-read.
      final phases = <Map<String, Object?>>[
        {'n': 0, 'roller': 'u', 'commit': hexA},
        {'n': 0, 'roller': 'u', 'commit': hexA},
        {'n': 0, 'roller': 'u', 'commit': hexA, 'entropy': hexB},
        {'n': 0, 'roller': 'u', 'commit': hexA, 'entropy': hexB, 'reveal': hexC},
      ];
      var rollCalls = 0;
      final cursors = <int>[];
      final api = await apiFor((req) async {
        final sq = (jsonDecode(req.body) as Map)['structuredQuery'] as Map;
        final collection = (sq['from'] as List)[0]['collectionId'] as String;
        if (collection == 'events') return http.Response('[]', 200);
        final cursor = int.parse(((sq['where'] as Map)['fieldFilter']
            as Map)['value']['integerValue'] as String);
        cursors.add(cursor);
        final body = rollCalls < phases.length
            ? rollRows([phases[rollCalls]])
            : '[]';
        rollCalls++;
        return http.Response(body, 200);
      });
      ...
      ], reason: 'the unchanged second poll must not re-emit');
      expect(cursors.take(4), [0, 0, 0, 0]);
      expect(cursors[4], 1, reason: 'the completed roll is retired');
    });

    test('forwards a transport error and keeps polling', () async {
      var calls = 0;
      final api = await apiFor((req) async {
        final sq = (jsonDecode(req.body) as Map)['structuredQuery'] as Map;
        if ((sq['from'] as List)[0]['collectionId'] == 'rolls') {
          return http.Response('[]', 200);
        }
        calls++;
        if (calls == 1) return err(403, 'PERMISSION_DENIED');
        return http.Response(eventRows([(0, DoubleEvent(Player.white))]), 200);
      });
  ...
  group('pollEvents', () {
    test('flattens the poll stream to individual events', () async {
      var calls = 0;
      final api = await apiFor((req) async {
        final sq = (jsonDecode(req.body) as Map)['structuredQuery'] as Map;
        if ((sq['from'] as List)[0]['collectionId'] == 'rolls') {
        ...
        if (calls++ > 0) return http.Response(eventRows([]), 200);
        return http.Response(
          eventRows([
      ...
      await sub.cancel();
      expect(seen.map((e) => e.seq), [0, 1]);
      expect(seen[1].event, TakeEvent(Player.black));
    });
  });
